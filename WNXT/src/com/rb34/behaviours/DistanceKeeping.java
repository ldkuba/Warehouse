...
public class DistanceKeeping implements Behavior {
	...
	public void action() {
		 ...
		 * constants' calculations as well as the loop. double kv = 15; double
		 * kp = (kv * 0.6); // The value of kp ki and kd were found using // the
		 * Ziegler�Nichols tuning method. double ki = (1.2 * (kv / delay));
		 * double kd = (0.6 * ((kv * delay) / 8)); double error = 0; double
		 * previousOutput = 0; maxSpeed = pilot.getMaxTravelSpeed();
		 * 
		 * output = Math.abs((kp * error) + (ki * (Math.abs(error) / delay)) +
		 * (kd * (Math.abs(error) / delay)));
		 * 
		 * if (output > maxSpeed) { output = maxSpeed - 0.05; } //
		 * System.out.println("New speed:" + output); Delay.msDelay(delay);
		 * //pilot.setTravelSpeed(output); pilot.rotate(90,true);
		 */
