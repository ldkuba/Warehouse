...
public class LineFollowing implements Behavior {
	...
	public void action() {
		...
		while (!supressed) {
			...
			/*
			 * if (!rightOnBlack() && !leftOnBlack()) { //if greater than 1 must
			 * be on white so forward pilot.forward();
			 * System.out.println("Forward"); Delay.msDelay(10); } else if
			 * (rightOnBlack() && !leftOnBlack()) { pilot.rotateRight();
			 * System.out.println("Recorrecting A"); Delay.msDelay(10); } else
			 * if (!rightOnBlack() && leftOnBlack()) { pilot.rotateLeft();
			 * System.out.println("Recorrecting B"); Delay.msDelay(10); }
			 */
			...
			/*
			 * LCD.clear(); LCD.drawInt(reading, 8, 1); //LCD.drawInt(whiteInit,
			 * 8, 3); //LCD.drawInt(whiteInit - reading, 8, 5);
			 * LCD.drawChar('R', 8, 7);
			 * 
			 * LCD.drawInt(readingL, 3, 1); //LCD.drawInt(whiteInitL, 3, 3);
			 * //LCD.drawInt(whiteInitL - readingL, 3, 5); LCD.drawChar('L', 3,
			 * 7); //checks values when trying to debug-prints on robot
			 */
			Delay.msDelay(30);
		}
	}
