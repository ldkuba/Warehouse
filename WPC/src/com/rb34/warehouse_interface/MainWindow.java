...
public class MainWindow extends Thread {
	...
	private void initialize() {
		...
		frame.add(btnWarehouseMap);
		btnWarehouseMap.addActionListener(e -> {
			new PathFrame().start();
		});
		...
		btnJobInformation.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				RobotInformation info = new RobotInformation(robotNum);
				info.run();
	...
	public void setStatus(JLabel val, int robotId) {
		String status = RobotManager.getRobot(robotId).getRobotStatus().toString();
		int rb = robotId + 1;
	...
	public void setCoords(JLabel val, int robotId) {
		String location = "(" + RobotManager.getRobot(robotId).getXLoc() + ","
				+ RobotManager.getRobot(robotId).getYLoc() + ")" + " Heading: "
				+ RobotManager.getRobot(robotId).getHeading();
		;
	...
	public void setJob(JLabel val, int robotId) {
		String jobID = "";
		if (RobotManager.getRobot(robotId).getCurrentJob().getJobId() == null) {
			jobID = "No Current Job";
		} else {
			jobID = RobotManager.getRobot(robotId).getCurrentJob().getJobId();
		}
